...
import { updateCard } from "../../services/moreInfo";
const tileDump = {
  text: "",
  title: "",
  position: 0,
...
export default function FormDialog(props) {
  ...
  const handleSubmit = () => {
    if (!title) return alert("Title field is required!");
    if (props.iconUrl != icon && !iconUploaded) return alert("Icon is required!")
    const body = {
      title,
      iconUrl: icon,
      tileData,
    ...
    updateCard({ cardId: props._id, data: body, token: localStorage.getItem("token") })
      .then((response) => {
        props.getCards();
        props.close();
      })
  ...
  return (
    <div>
      <Dialog fullWidth={true} open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
        <DialogTitle id="form-dialog-title">Edit Card/Info</DialogTitle>
        <DialogContent>
...

          {
            !iconUploaded &&
            (file && file.name ?
              <Button variant="contained" color="primary" onClick={() => uploadFile(file, "iconUrl")}>
                Upload Icon
            </Button>
              :
            ...
            <h4>Tile Data</h4>
            {
              tileData.map((tile, ind) => {
                return <TileDataRender
                  ...
                  handleZeroIndex={handleZeroIndex}
                />
              })
            }
          </div>
...
const TileDataRender = (props) => {
  const [iconUploaded, setIconUploaded] = useState(false);
  const [icon, setIcon] = useState(props.iconUlr);
  const [file, setFile] = useState();
  ...
  return (
    <div key={props.index} style={{ padding: "5px" }}>
      <Button onClick={() => props.index === 0 ? props.handleZeroIndex() : props.handleDrop(props.index)} color="secondary" variant="contained" >X</Button>
